...
template <typename State> struct EvalLog {
  ...
  static constexpr size_t frame_size =
      State::log_size + SIZE_BATTLE_WITH_PRNG + 3 + 2 + 2 + 2 * 4 * (19);
...
void rollout_with_eval_debug(State &state,
                             typename RowModelTypes::Model &row_model,
                             typename ColModelTypes::Model &col_model,
  ...
  while (!state.is_terminal()) {
    ...
    const int frame_size = apply_actions_with_eval_log(
        state, row_action, col_action, &row_output, &col_output, data);
...
void self_play_rollout_with_eval_debug(prng &device, State &state,
                                       typename ModelTypes::Model &model,
                                       EvalLog<State> &debug_log) {
  ...
  while (!state.is_terminal()) {
    ...
    const int frame_size = apply_actions_with_eval_log(
        state, row_action, col_action, &output,
        static_cast<typename ModelTypes::ModelOutput *>(nullptr), data);
    debug_log.frame_sizes.push_back(frame_size);
