...
template <typename T> constexpr bool fuzzy_equals(const T a, const T b) {
...
template <typename Types, bool debug = false> struct AlphaBetaRefactor : Types {
  ...
  template <typename... T> static void debug_print(T... args) {
    if constexpr (debug) {
  ...
  template <typename... T> static void debug_print_no_indent(T... args) {
    if constexpr (debug) {
      ((std::cout << args), ...);
    }
  }
  ...
  struct BaseData {
    ...
    BaseData(uint32_t max_depth, PRNG *device, Model *model, uint32_t min_tries,
             std::vector<uint32_t> max_tries, float max_unexplored,
             float min_reach_prob)
        : max_depth{max_depth}, device{device}, model{model},
          min_tries{min_tries}, max_tries{max_tries},
          max_unexplored{max_unexplored}, min_reach_prob{min_reach_prob} {}
  };
  ...
  struct HeadData {
    ...
    HeadData(uint32_t min_tries, uint32_t max_tries)
        : min_tries{min_tries}, max_tries{max_tries} {}

    void step_forward() { ++depth; }
    void step_back() { --depth; }
  };
  ...
  static void debug_print_chance_matrix(const TempData &temp_data) {
    ...
    for (uint8_t row_idx = 0; row_idx < temp_data.rows; ++row_idx) {
      ...
      for (uint8_t col_idx = 0; col_idx < temp_data.cols; ++col_idx) {
        ...
        debug_print_no_indent(data.alpha_explored, " - ", data.beta_explored,
                              "|", data.unexplored, ", ");
        ++entry_idx;
      }
  ...
  struct MatrixNode {
    ...
    void prune() {
      for (uint8_t i = I.boundary; i < chance_node_matrix.rows; ++i) {
        for (uint8_t j = J.boundary; j < chance_node_matrix.cols; ++j) {
          auto &chance_node = chance_node_matrix(I.action_indices[i].idx,
                                                 J.action_indices[j].idx);
    ...
    void sort_branches_and_reset() {
      for (uint8_t i = 0; i < chance_node_matrix.rows * chance_node_matrix.cols;
           ++i) {
        // because the branches are added by sampling, this array should be
        // mostly sorted on its own idk if std::sort takes full advantage of
        // this, and that only a tail of the array is unsorted;
        // TODO
        ...
        std::sort(chance_node.sorted_branches.begin(),
  ...
  struct Search {
    ...
    Search(const uint32_t min_tries, const uint32_t max_tries,
           const float max_unexplored, const float min_reach_prob_initial,
           const float min_reach_prob_base)
        : min_tries{min_tries}, max_tries{max_tries},
          max_unexplored{max_unexplored},
          min_reach_prob_initial{min_reach_prob_initial},
          min_reach_prob_base{min_reach_prob_base} {}

    Search(

    ) {}

    // calls alpha_beta (basically a wrapper for it) but first checks if not
    // terminal/at max depth helps with boiler plate and reduces matrix node
    // allocations
    void next_solve(TempData::ChanceStats &stats, Branch *branch,
                    BaseData &base_data, HeadData &head_data,
                    TempData &temp_data, TempData &next_temp_data) const {

      head_data.step_forward();
      const float prob = next_temp_data.state.get_prob();
      if (next_temp_data.state.is_terminal()) {
        debug_print("next_solve: terminal: ",
                    next_temp_data.state.get_payoff().get_row_value(), '\n');
        next_temp_data.alpha = next_temp_data.beta =
            next_temp_data.state.get_payoff().get_row_value();
        ++base_data.terminal_count;
      } else if ((next_temp_data.chance_reach_prob =
                      temp_data.chance_reach_prob * prob) <
                     base_data.min_reach_prob ||
                 head_data.depth == base_data.max_depth) {
        // if (next_temp_data.chance_reach_prob < base_data.min_reach_prob) {
        //     std::cout << next_temp_data.chance_reach_prob << " " <<
        //     base_data.min_reach_prob << std::endl;
        // }
        ModelOutput output{};
        ...
        this->alpha_beta(branch->matrix_node.get(), base_data, head_data,
                         next_temp_data);
        if constexpr (debug) {
          indent -= 4;
        }
    ...
    // branches stored in the tree. then it tries with brute force until
    // max_tries, max_unplored, etc
    Branch *try_get_new_branch(const BaseData &base_data,
                               const HeadData &head_data,
                               const TempData &temp_data,
                               TempData &next_temp_data,
                               ChanceNode *chance_node, const uint8_t row_idx,
                               const uint8_t col_idx) const {
      ...
      auto &chance_data =
          temp_data.chance_stat_matrix[row_idx * temp_data.cols + col_idx];
      while (!(chance_node->tries >= base_data.max_tries[head_data.depth] ||
               (chance_node->tries > head_data.min_tries &&
                chance_data.unexplored < base_data.max_unexplored) ||
        ...
        if (chance_node->branches.find(obs_hash) ==
            chance_node->branches.end()) {
          chance_node->branches.try_emplace(
              obs_hash, next_temp_data.state.get_prob(), seed,
              next_temp_data.state.is_terminal());
          chance_node->sorted_branches.push_back(
              &chance_node->branches.at(obs_hash));
    ...
    void solve_chance_node(MatrixNode *matrix_node, BaseData &base_data,
                           HeadData &head_data, TempData &temp_data,
                           TempData &next_temp_data, const uint8_t row_idx,
                           const uint8_t col_idx) const {
      ...
      ChanceNode *chance_node =
          &matrix_node->chance_node_matrix(row_idx, col_idx);
      while (new_branch = try_get_new_branch(base_data, head_data, temp_data,
                                             next_temp_data, chance_node,
                                             row_idx, col_idx)) {
        next_solve(chance_data, new_branch, base_data, head_data, temp_data,
                   next_temp_data);
        temp_data.is_subgame_singular &=
            (next_temp_data.alpha == next_temp_data.beta);
      }
      temp_data.is_subgame_singular &= (chance_data.unexplored == 0);
    }

    void discretize_strategies(MatrixNode *matrix_node,
                               const TempData &temp_data) const {
    ...
    compute_subgame_equilibrium(MatrixNode *matrix_node, BaseData &base_data,
                                HeadData &head_data, TempData &temp_data,
                                TempData &next_temp_data) const {
      ...
      if (temp_data.is_subgame_singular) {
        ...
        for (uint8_t i = 0; i < r; ++i) {
          ...
          for (uint8_t j = 0; j < c; ++j) {
            const auto &data =
                temp_data
                    .chance_stat_matrix[matrix_node->I.action_indices[i].idx *
                                            temp_data.cols +
                                        matrix_node->J.action_indices[j].idx];
            temp_data.alpha_matrix[entry_idx] =
                static_cast<int>(data.alpha_explored * solve_denominator);
        ...
        for (uint8_t i = 0; i < r; ++i) {
          ...
          for (uint8_t j = 0; j < c; ++j) {
            ...
            temp_data.alpha_matrix[entry_idx] =
                static_cast<int>(data.alpha_explored * solve_denominator);
            temp_data.beta_matrix[entry_idx] = static_cast<int>(
                (data.beta_explored + data.unexplored) * solve_denominator);
            debug_print_no_indent(temp_data.alpha_matrix[entry_idx], " - ",
                                  temp_data.beta_matrix[entry_idx], "|",
                                  data.unexplored, ", ");
            ++entry_idx;
          }
          debug_print_no_indent('\n');
    ...
    void initialize_submatrix(MatrixNode *matrix_node, BaseData &base_data,
                              HeadData &head_data, TempData &temp_data,
                              TempData &next_temp_data) const {
      ...
      for (uint8_t i = 0; i < matrix_node->I.boundary; ++i) {
        ...
        for (uint8_t j = 0; j < matrix_node->J.boundary; ++j) {
          ...
          solve_chance_node(matrix_node, base_data, head_data, temp_data,
                            next_temp_data, row_idx, col_idx);
        }
      }
    }

    void find_row_best_response(float &best_response, MatrixNode *matrix_node,
                                BaseData &base_data, HeadData &head_data,
                                TempData &temp_data,
                                TempData &next_temp_data) const {
      ...
      const auto find_nonzero_priority_index = [&](uint8_t &max_index,
                                                   const BestResponse &data) {
        float max_prio{-1};
      ...
      const auto compute_value = [&](BestResponse &data, uint8_t curr_i) {
        ...
        data.priority_vector.resize(matrix_node->J.boundary);
        {
          for (uint8_t j = 0; j < matrix_node->J.boundary; ++j) {
            ...
            const auto &chance_stats =
                temp_data.chance_stat_matrix[data.row_idx * temp_data.cols +
                                             data.col_idx];

            data.priority_vector[j] =
                chance_stats.unexplored * temp_data.col_strategy[j];
            data.total_unexplored += data.priority_vector[j];
            data.value +=
                chance_stats.beta_explored * temp_data.col_strategy[j];
          }
        }
        ...
        Branch *branch;
        for (uint8_t j; can_beat_best_response(data) &&
                        find_nonzero_priority_index(j, data);) {
          data.col_idx = matrix_node->J.action_indices[j].idx;
          ChanceNode *chance_node =
              &matrix_node->chance_node_matrix(data.row_idx, data.col_idx);
          if (branch = try_get_new_branch(base_data, head_data, temp_data,
                                          next_temp_data, chance_node,
                                          data.row_idx, data.col_idx)) {
            const float prob = next_temp_data.state.get_prob();
            auto &chance_data =
                temp_data.chance_stat_matrix[data.row_idx * temp_data.cols +
                                             data.col_idx];
            next_solve(chance_data, branch, base_data, head_data, temp_data,
                       next_temp_data);
            const float p = prob * temp_data.col_strategy[j];
            data.priority_vector[j] -= p;
            data.value +=
                next_temp_data.beta * prob * temp_data.col_strategy[j];
        ...
        if (data.value > best_response ||
            (fuzzy_equals(data.value, best_response) &&
             curr_i >= matrix_node->I.boundary)) {
          temp_data.new_row_idx = data.row_idx;
          temp_data.new_i = curr_i;
          best_response = std::move(data.value);
          debug_print("new best response - new_i: ", (int)temp_data.new_i,
                      " new_row_idx: ", (int)temp_data.new_row_idx, '\n');
        }
      };
      ...
      if (temp_data.new_i >= matrix_node->I.boundary) {
        for (uint8_t j = 0; j < matrix_node->J.boundary; ++j) {
          this->solve_chance_node(matrix_node, base_data, head_data, temp_data,
                                  next_temp_data, temp_data.new_row_idx,
                                  matrix_node->J.action_indices[j].idx);
        }
    ...
    void find_col_best_response(float &best_response, MatrixNode *matrix_node,
                                BaseData &base_data, HeadData &head_data,
                                TempData &temp_data,
                                TempData &next_temp_data) const {
        ...
        data.priority_vector.resize(matrix_node->I.boundary);
        {
          for (uint8_t i = 0; i < matrix_node->I.boundary; ++i) {
            ...
            data.priority_vector[i] =
                chance_stats.unexplored * temp_data.row_strategy[i];
            data.total_unexplored += data.priority_vector[i];
            data.value +=
                chance_stats.alpha_explored * temp_data.row_strategy[i];
            // data.value;
          }
        }

        Branch *branch;
        for (uint8_t i; can_beat_best_response(data) &&
                        find_nonzero_priority_index(i, data);) {
          data.row_idx = matrix_node->I.action_indices[i].idx;
          ChanceNode *chance_node =
              &matrix_node->chance_node_matrix(data.row_idx, data.col_idx);
          if (branch = try_get_new_branch(base_data, head_data, temp_data,
      ...
      if (temp_data.new_j >= matrix_node->J.boundary) {
        for (uint8_t i = 0; i < matrix_node->I.boundary; ++i) {
          this->solve_chance_node(
              matrix_node, base_data, head_data, temp_data, next_temp_data,
    ...
    void alpha_beta(MatrixNode *matrix_node, BaseData &base_data,
      ...
      // only allocate for all next function calls, modify in other functions
      // when necessary
      TempData next_temp_data;

      debug_print("ALPHA BETA - depth: ", head_data.depth, '\n');
      debug_print("initializing submatrix");
      this->initialize_submatrix(matrix_node, base_data, head_data, temp_data,
                                 next_temp_data);

      while (!fuzzy_equals(temp_data.alpha, temp_data.beta) &&
             !temp_data.must_break) {
        ...
        auto [next_alpha, next_beta] = this->compute_subgame_equilibrium(
            matrix_node, base_data, head_data, temp_data, next_temp_data);

        if constexpr (debug) {
          indent += 2;
        }
        this->find_row_best_response(next_beta, matrix_node, base_data,
                                     head_data, temp_data, next_temp_data);
        this->find_col_best_response(next_alpha, matrix_node, base_data,
                                     head_data, temp_data, next_temp_data);
        if constexpr (debug) {
          indent -= 2;
        }
        // each of the two prior function calls solves most new chance nodes,
        // but they both miss this corner case when two new actions are added
        // TODO this looks too much like the old code lol
        if (temp_data.new_i >= matrix_node->I.boundary &&
            temp_data.new_j >= matrix_node->J.boundary) {
          debug_print("float new actions: ", (int)temp_data.new_row_idx, ' ',
                      (int)temp_data.new_col_idx, '\n');
          this->solve_chance_node(matrix_node, base_data, head_data, temp_data,
                                  next_temp_data, temp_data.new_row_idx,
                                  temp_data.new_col_idx);
        }
        if (temp_data.new_i >= matrix_node->I.boundary) {
        ...
        debug_print("new row idx: ", (int)temp_data.new_row_idx,
                    " new col idx: ", (int)temp_data.new_col_idx,
                    " must_break:", temp_data.must_break, '\n');
        debug_print("________________", '\n');
        debug_print('\n');
      }
      ...
      // temp_data.alpha = temp_data.beta = (temp_data.alpha + temp_data.beta
      // )/2;
    }
    ...
    template <typename T>
    Output run(const std::vector<T> &depths, PRNG &device, const State &state,
               Model &model, MatrixNode &node) const {
      ...
      for (const auto depth : depths) {
        ...
        BaseData base_data{depth,         &device,       &model,
                           min_tries,     max_tries_vec, max_unexplored,
                           min_reach_prob};
    ...
    Output run(const uint32_t depth, PRNG &device, const State &state,
               Model &model, MatrixNode &node) const {
