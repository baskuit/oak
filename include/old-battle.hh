    ...
    public:
            ...
            W::Types::ModelOutput *col_output = nullptr)
        {
            ...
            if constexpr (use_debug_log)
            {
                ...
                if (row_output != nullptr)
                {
                    ...
                    debug_log.push_back(static_cast<uint8_t>(row_output->value.get_row_value() * 255));
                    debug_log.push_back(n);
                    for (size_t i = 0; i < n; ++i)
                    {
                        debug_log.push_back(this->row_actions[i]);
                        debug_log.push_back(static_cast<uint8_t>(row_output->row_policy[i] * 255));
                    }
                }
                else
                {
                ...
                if (col_output != nullptr)
                {
                    ...
                    debug_log.push_back(static_cast<uint8_t>(col_output->value.get_col_value() * 255));
                    debug_log.push_back(n);
                    for (size_t i = 0; i < n; ++i)
                    {
                        debug_log.push_back(this->col_actions[i]);
                        debug_log.push_back(static_cast<uint8_t>(col_output->col_policy[i] * 255));
                    }
                }
                else
                {
        ...
        void save_debug_log() const
        {
            ...
            const std::string path =
                "/home/user/pkmn-pinyon-test/logs/" +
                p1_id + "_" + p2_id + "_" + std::to_string(static_cast<int>(this->payoff.get_row_value() * 2)) + "_" +
