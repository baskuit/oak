...
template <typename Types> struct ModelOutputImpl<Types, false> {
  Types::Value value;
};

template <typename Types> struct ModelOutputImpl<Types, true> {
  Types::Value value;
...
struct MonteCarloModelAverage : Types {
  using ModelOutput =
      MonteCarloModelAverageDetail::ModelOutputImpl<Types, has_policy>;
  ...
  public:
    ...
    Model(const Types::PRNG &device, const size_t tries = 1)
    ...
    void inference(Types::State &&state, ModelOutput &output) {
      if constexpr (has_policy) {
        ...
        const typename Types::Real row_uniform{
            Rational{1, static_cast<int>(rows)}};
        output.row_policy.resize(rows, row_uniform);
        const typename Types::Real col_uniform{
            Rational{1, static_cast<int>(cols)}};
    ...
    void inference(ModelBatchInput &batch_input,
    ...
    void add_to_batch_input(Types::State &&state,
                            ModelBatchInput &input) const {
      input.push_back(state);
    }

    void get_output(ModelOutput &model_output,
