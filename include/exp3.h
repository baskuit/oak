...
template <IsValueModelTypes Types> struct Exp3Oak : Types {
  ...
  struct MatrixStats {
    ...
    static_assert(
        std::is_base_of_v<std::array<Real, 9>, typename Types::VectorReal>);
  };
  struct ChanceStats {};
  struct Outcome {
    int row_idx, col_idx;
  ...
  public:
    ...
    constexpr BanditAlgorithm(Real gamma)
        : gamma(gamma), one_minus_gamma{gamma * -1 + 1} {}
    ...
    void get_empirical_strategies(const MatrixStats &stats,
      ...
      // math::power_norm(stats.row_visits, row_strategy.size(), 1,
      // row_strategy); math::power_norm(stats.col_visits, col_strategy.size(),
      // 1, col_strategy);
    }

    void get_empirical_value(const MatrixStats &stats,
                             Types::Value &value) const {
      // const Real den = typename Types::Q{1, (stats.visits + (stats.visits ==
      // 0))}; if constexpr (Types::Value::IS_CONSTANT_SUM) {
      //     value = typename
      //     Types::Value{Real{stats.value_total.get_row_value() * den}};
      // } else {
    ...
    void get_refined_strategies(const MatrixStats &stats,
      ...
      // math::power_norm(stats.row_visits, row_strategy.size(), 1,
      // row_strategy); math::power_norm(stats.col_visits, col_strategy.size(),
      // 1, col_strategy); denoise(row_strategy, col_strategy);
    }
    ...
    void expand(MatrixStats &stats, const size_t &rows, const size_t &cols,
      ...
      // std::fill(std::begin(stats.row_gains), std::begin(stats.row_gains) +
      // rows, 0); std::fill(std::begin(stats.col_gains),
      // std::begin(stats.col_gains) + cols, 0);
    }
    ...
    void select(Types::PRNG &device, const MatrixStats &stats,
      ...
      if (rows == 1) {
        ...
        std::transform(row_forecast.begin(), row_forecast.begin() + rows,
                       row_forecast.begin(),
      ...
      if (cols == 1) {
        ...
        std::transform(col_forecast.begin(), col_forecast.begin() + cols,
                       col_forecast.begin(),
                       [eta, one_minus_gamma](Real value) {
                         return one_minus_gamma * value + eta;
                       });
    ...
    void update_matrix_stats(MatrixStats &stats, const Outcome &outcome) const {
      // stats.value_total += PairReal<Real>{outcome.value.get_row_value(),
      // outcome.value.get_col_value()}; stats.visits += 1;
      // stats.row_visits[outcome.row_idx] += 1;
      // stats.col_visits[outcome.col_idx] += 1;
      if ((stats.row_gains[outcome.row_idx] +=
           outcome.value.get_row_value() / outcome.row_mu) >= 0) {
        const auto max = stats.row_gains[outcome.row_idx];
        for (auto &v : stats.row_gains) {
          v -= max;
        }
      }
      if ((stats.col_gains[outcome.col_idx] +=
           outcome.value.get_col_value() / outcome.col_mu) >= 0) {
    ...
    void update_matrix_stats(MatrixStats &stats, const Outcome &outcome,
    ...
    void update_chance_stats(ChanceStats &stats, const Outcome &outcome,
                             Types::Mutex &mutex) const {}

  private:
    template <typename GainsVector>
    inline void softmax(Types::VectorReal &forecast, const GainsVector &gains,
                        const size_t k, Real eta) const {
      Real sum = 0;
      for (size_t i = 0; i < k; ++i) {
        const Real y{std::exp(static_cast<float>(
            gains[i] * eta))}; // TODO maybe but T::Float back
        forecast[i] = y;
        sum += y;
      }
      for (size_t i = 0; i < k; ++i) {
    ...
    inline void denoise(Types::VectorReal &row_strategy,
                        Types::VectorReal &col_strategy) const {
      ...
      if (rows > 1) {
        const Real eta{gamma / static_cast<Real>(rows)};
        std::transform(row_strategy.begin(), row_strategy.begin() + rows,
                       row_strategy.begin(),
                       [eta, one_minus_gamma](Real value) {
      ...
      if (cols > 1) {
        const Real eta{gamma / static_cast<Real>(cols)};
        std::transform(col_strategy.begin(), col_strategy.begin() + cols,
                       col_strategy.begin(),
                       [eta, one_minus_gamma](Real value) {
                         return (value - eta) / one_minus_gamma;
                       });
