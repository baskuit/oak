...
public:
  ...
  void update(const Outcome &outcome) noexcept {
    ...
    if ((col_gains[outcome.col_idx] += (1 - outcome.value) / outcome.col_mu) >= 0) {
  ...
  void select(PRNG& device, Outcome &outcome) const noexcept {
    ...
    if (_rows == 1) {
      ...
      std::transform(row_forecast.begin(), row_forecast.begin() + _rows, row_forecast.begin(),
                      [eta, one_minus_gamma](const float value) { return one_minus_gamma * value + eta; });
    ...
    if (_cols == 1) {
      ...
      std::transform(col_forecast.begin(), col_forecast.begin() + _cols, col_forecast.begin(),
                      [eta, one_minus_gamma](const float value) { return one_minus_gamma * value + eta; });
...
private:
  void softmax(auto &forecast, const auto &gains, const auto k, float eta) const {
      float sum = 0;
      for (auto i = 0; i < k; ++i) {
