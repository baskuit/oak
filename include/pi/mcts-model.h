...
template <bool debug_print = false, bool clamp_rolls = true> struct MCTSEval {
  ...
  auto run(const auto iterations, auto &prng, auto &node,
           const pkmn_gen1_battle &battle, const pkmn_result result,
           const pkmn_gen1_chance_durations &durations, const auto &model = 0) {
    ...
    for (auto iteration = 0; iteration < iterations; ++iteration) {
      ...
      *battle_seed = prng.uniform_64();
      ...
      total_value += run_iteration<enable_visit_matrix>(
          prng, &node, battle_copy, result, model);
    }
  ...
  float run_iteration(auto &prng, auto *node, pkmn_gen1_battle &battle,
                      pkmn_result result, Abstract::Battle &abstract,
                      const auto &model, size_t depth = 0) {
    ...
    if (node->stats().is_init()) {
      ...
      outcome.value = run_iteration<enable_visit_matrix>(
          prng, child, battle, result, model, depth + 1);
