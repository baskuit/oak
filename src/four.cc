...
void print_output(const State &state, const Output &output) {
  std::cout << "alpha: " << output.alpha << " beta: " << output.beta
            << std::endl;
  std::cout << "terminal: ";
...
template <typename Types> struct BattleSearchModel : Types {
  struct ModelOutput {
  ...
  public:
    ...
    Model(const prng &device)
        : device{device}, model{this->device.uniform_64()} {}
...
State generator(prng &device, const int max_alive_side,
                const float use_prob = .33) {
  /*
  uses n_alive to create search-tractable states
  */

  const int r = device.random_int(100);
  const int c = device.random_int(100);
  ...
  while (!state.is_terminal()) {
    ...
    if (m <= max_alive_side && n <= max_alive_side && (rows > 1 && cols > 1) &&
        device.uniform() < use_prob) {
      state.clamped = true;
      return state;
    }
  }
...
template <typename T> void print_matrix_data(const T &matrix_data) {
  ...
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      ...
      const int y = b * 1000;
      const int z = c * 1000;
      std::cout << "(" << tostring(x) << " " << tostring(y) << "|"
                << tostring(z) << " " << tostring(d) << ") ";
    }
    std::cout << std::endl;
  }
}
...
template <typename Types> void bar(const prng &device_) {
  prng device{device_};
  typename Types::State state =
      generator<typename Types::State>(device, max_alive);
  typename Types::Model model{};
  typename Types::MatrixNode node{};
  typename Types::Search search{min_tries, max_tries, max_unexplored,
...
template <typename Types> void bar2(const prng &device_) {
  prng device{device_};
  typename Types::State state =
      generator<typename Types::State>(device, max_alive);
...
int foo(prng &device) {
  ...
  using P = AlphaBetaRefactor<PModel<Battle<0, 3, ChanceObs, float, float>>,
                              debug_print>;
  using T =
      AlphaBetaRefactor<MonteCarloModel<Battle<0, 3, ChanceObs, float, float>>,
                        debug_print>;
  using U = AlphaBetaRefactor<
      BattleSearchModel<Battle<0, 3, ChanceObs, float, float>>, debug_print>;
  using V = AlphaBetaRefactor<
      MonteCarloModelAverage<Battle<0, 3, ChanceObs, float, float>>,
      debug_print>;
