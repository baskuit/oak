...
bool read_params_from_dir(std::filesystem::path path, auto &pokemon_net,
  ...
  const auto read_embedding_net = [](auto &nn, const std::filesystem::path path,
  ...
  const auto read_main_net = [](auto &nn, const std::filesystem::path path,
                       const std::string tag) {
    std::ifstream accs, w0s, b0s, w1s, b1s, w2s, b2s;
    std::filesystem::path p = path / (tag + "w0");
    w0s.open(path / (tag + "w0"), std::ios::binary);
    b0s.open(path / (tag + "b0"), std::ios::binary);
    w1s.open(path / (tag + "vw1"), std::ios::binary);
    b1s.open(path / (tag + "vb1"), std::ios::binary);
    w2s.open(path / (tag + "vw2"), std::ios::binary);
    b2s.open(path / (tag + "vb2"), std::ios::binary);
    const bool success = nn.fc_0.read_parameters(w0s, b0s) &&
                         nn.fc_1.read_parameters(w1s, b1s) &&
                         nn.fc_2.read_parameters(w2s, b2s);
    return success;
  };
  const bool m_success = read_main_net(main_net, path, "nn");  
  const bool p_success = read_embedding_net(pokemon_net, path, "p");
  const bool a_success = read_embedding_net(active_net, path, "a");
  return m_success && p_success && a_success;
}

void thread_fn(std::atomic<size_t> *score, std::atomic<size_t> *counter,
              size_t iterations, std::string net_path) {
...
int main(int argc, char **argv) {
  if (argc < 4) {
    std::cerr << "Input: net-path, threads, iterations." << std::endl;
    return 1;
  }
  ...
  const size_t iterations = std::atoll(argv[3]);
  ...
  for (auto t = 0; t < threads; ++t) {
    thread_pool[t] = std::thread{&thread_fn, &score, &counter, iterations, net_path};
  }
  ...
  std::cout << (double)score.load() / 2 / counter.load() << std::endl;
