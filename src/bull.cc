...
std::ofstream OUTPUT_FILE{"fb-vs-standard.txt", std::ios::out | std::ios::trunc};
    ...
    const State &state)
{
    ...
    for (int row_idx = 0; row_idx < rows; ++row_idx)
    {
        for (int col_idx = 0; col_idx < cols; ++col_idx)
        {
            ...
            entry.payoff_matrix[row_idx][col_idx] = payoff_matrix.get(row_idx, col_idx).get_row_value().get_d();
        }
    }
