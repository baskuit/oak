...
void q_value() {
    ...
    using LeafModel =
        Clamped<SearchModel<AlphaBetaForce<MonteCarloModelAverage<B>>,
                            false, false, false>>;
    ...
    std::cout << "tries: " << mapping_tries << "; " << mapped_state.node->count_matrix_nodes() << " nodes."
              << std::endl;
    ...
    // for (int i = 0; i < 4; ++i) {
    // for (int j = 0; j < 4; ++j) {
    //     float_matrix.get(i, j) = math::to_float(node.stats.nash_payoff_matrix.get(i, j).get_row_value());
    // }}
    // float_matrix.print();
}
